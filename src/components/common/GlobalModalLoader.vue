<template>
  <Teleport to="body">
    <div v-if="modalStore.stack.length > 0" class="fixed inset-0 z-[3000]">
      <!-- Backdrop -->
      <div
      ...
      <!-- Render all modals in the stack, centered and stacked. Above
      z-[2000] — the highest z-index any locally-built modal (e.g. the
      booking payment popup) uses — so a ConfirmModal/StatusModal opened
      from inside one of those always renders on top of it, not hidden
      behind it. -->
      <div
        v-for="(modal, index) in modalStore.stack"
        :key="modal.id"
        class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none"
        :style="{ zIndex: 3001 + index }"
      >
